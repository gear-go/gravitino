...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 10 13:15:05 2018
...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 14 15:26:24 2017
...
###################################################################################################
"""
Function to generate lepton and photon spectra from gravitino
...
#        return positrons(Ce,gammae,Cp, gammap, Cs, gammas, Esi) / ( positrons(Ce,gammae,Cp, gammap, Cs, gammas, Esi) + electrons(Ce, gammae,Cp, gammap, Cs, gammas, Esi) )
    return spectrum(epp_CALET_data['Ener'])
...
def loglike(cube,ndim,nparams):
        Cp, gammap,Ce, gammae, Mdm, logfluxNorm, a1, a2, logg, logh, logi=cube[0], cube[1], cube[2], cube[3], cube[4],cube[5], cube[6], cube[7], cube[8], cube[9], cube[10]
        ...
        #yerreppc = epp_CALET_data['ERR_STATma']
        #yeppc = epp_CALET_data['VALUE']
...
ypdata = p_data['VALUE']
yedata = e_data['VALUE']
datafile = "fit_multines_C_Gravitino_NoSum_shade"
parameters = ["Cp", "gammap", "Ce", "gammae", "Mdm", "logfluxNorm", "a1", "a2", "g", "h", "i"]
...
###############PLOT Positrons flux FROM FIT TO ELECTRONS AND POSITRONS#############c1 = np.array(a1,0,0)
...
x = p_data['Ener']
shade = Shade(x)
for (Cp, gammap, Ce, gammae, Mdm, logfluxNorm, a1, a2, g, h, i) in a.get_equal_weighted_posterior()[::10,:-1]:
...
x = epp_CALET_data['Ener']
shade = Shade(x)
for (Cp, gammap, Ce, gammae, Mdm, logfluxNorm, a1, a2, g, h, i) in a.get_equal_weighted_posterior()[::10,:-1]:
	shade.add((epp_CALET_data['Ener']**3)*suma(Cp,gammap,Ce,gammae, Mdm, 10**logfluxNorm, a1, a2))
shade.line(color='k')
shade.shade(q=0.25, color='#008837', alpha=0.3)
shade.shade(q=0.5, color='#7b3294', alpha=0.3)
...
sc = plt.scatter(values[:,4],1e26/(10**values[:,5]),c=values[:,11], vmin=min(values[:,11]), vmax=max(values[:,11]), marker="o",alpha=0.5, label="all points")
...
#plt.plot(p_data['Ener'], (p_data['Ener']**3)*positrons(ml_Cp, ml_gammap, ml_Cs, ml_gammas, ml_Esi),color='red',label='model fitted')
